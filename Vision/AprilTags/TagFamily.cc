...
TagFamily::TagFamily(int bitsArg, int minHammingDistanceArg, const unsigned long long *codesArg)
  : blackBorder(1), bits(bitsArg), dimension((int)std::sqrt((float)bits)),
    minimumHammingDistance(minHammingDistanceArg), errorRecoveryBits(1), codes() {
  if ( bits != dimension*dimension )
    cerr << "Error: TagFamily constructor called with bits=" << bits << "; must be a square number!" << endl;
  unsigned int length = 0;
  while ( codesArg[length] ) ++length;
  codes.resize(length);
  for (unsigned int i = 0; i < length; i++)
    codes[i] = codesArg[i];
  tagFamilyRegistry[std::pair<int,int>(bits,minimumHammingDistance)] = this;
}
...
std::map<std::pair<int,int>, TagFamily*> TagFamily::tagFamilyRegistry;
