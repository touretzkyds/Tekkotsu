...
Real GeneralMatrix::SumSquare() const
{
   ...
   (const_cast<GeneralMatrix&>(*this)).tDelete(); return sum;
}
...
{
   (const_cast<GeneralMatrix&>(*gm)).tDelete();
...
Real GeneralMatrix::MaximumAbsoluteValue() const
{
   ...
   (const_cast<GeneralMatrix&>(*this)).tDelete(); return maxval;
}
...
Real GeneralMatrix::MinimumAbsoluteValue() const
{
   ...
   (const_cast<GeneralMatrix&>(*this)).tDelete(); return minval;
}
...
Real GeneralMatrix::MaximumAbsoluteValue2(int& i, int& j) const
{
   ...
   MatrixRow mr(const_cast<GeneralMatrix*>(this), LoadOnEntry+DirectPart);
   for (int r = 1; r <= nr; r++)
   {
...
Real SymmetricMatrix::SumSquare() const
{
   ...
   (const_cast<SymmetricMatrix&>(*this)).tDelete(); return sum1 + 2.0 * sum2;
}
...
Real IdentityMatrix::SumSquare() const
{
   ...
   (const_cast<IdentityMatrix&>(*this)).tDelete(); return sum;
}


Real BaseMatrix::SumSquare() const
{
   REPORT GeneralMatrix* gm = (const_cast<BaseMatrix&>(*this)).Evaluate();
...
//   while (i--) { sum += *s; s += d; }
   ...
   (const_cast<Matrix&>(*this)).tDelete(); return sum;
}
...
Real DiagonalMatrix::Trace() const
{
   ...
   while (i--) sum += *s++;
   (const_cast<DiagonalMatrix&>(*this)).tDelete(); return sum;
}
...
Real SymmetricMatrix::Trace() const
{
   ...
   (const_cast<SymmetricMatrix&>(*this)).tDelete(); return sum;
}
...
Real LowerTriangularMatrix::Trace() const
{
   ...
   // while (i--) { sum += *s; s += j++; }
   if (i) for (;;) { sum += *s; if (!(--i)) break; s += j++; }
   (const_cast<LowerTriangularMatrix&>(*this)).tDelete(); return sum;
}
...
Real UpperTriangularMatrix::Trace() const
{
   ...
   (const_cast<UpperTriangularMatrix&>(*this)).tDelete(); return sum;
}
...
Real BandMatrix::Trace() const
{
   ...
   (const_cast<BandMatrix&>(*this)).tDelete(); return sum;
}
...
Real SymmetricBandMatrix::Trace() const
{
   ...
   // while (i--) { sum += *s; s += w; }
   if (i) for (;;) { sum += *s; if (!(--i)) break; s += w; }
   (const_cast<SymmetricBandMatrix&>(*this)).tDelete(); return sum;
}
...
Real BaseMatrix::Trace() const
{
   ...
   GeneralMatrix* gm = (const_cast<BaseMatrix&>(*this)).Evaluate(Diag);
...
: gm( ( (const_cast<BaseMatrix&>(bm)).Evaluate() )->MakeSolver() )
{
