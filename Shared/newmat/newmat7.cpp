...
{
   REPORT
   gm2 = (const_cast<BaseMatrix*&>(bm2))->Evaluate();
   gm2 = gm2->Evaluate(gm2->Type().MultRHS());     // no symmetric on RHS
   gm1=(const_cast<BaseMatrix*&>(bm1))->Evaluate();
...
GeneralMatrix* AddedMatrix::Evaluate(MatrixType mtd)
{
   ...
   gm1=(const_cast<BaseMatrix*&>(bm1))->Evaluate(); gm2=(const_cast<BaseMatrix*&>(bm2))->Evaluate();
...
Real BaseMatrix::Norm1() const
{
   ...
   REPORT
   GeneralMatrix* gm = (const_cast<BaseMatrix&>(*this)).Evaluate();
...
bool operator==(const BaseMatrix& A, const BaseMatrix& B)
{
   ...
   REPORT
   GeneralMatrix* gmA = (const_cast<BaseMatrix&>(A)).Evaluate();
   GeneralMatrix* gmB = (const_cast<BaseMatrix&>(B)).Evaluate();
...
bool IsZero(const BaseMatrix& A)
{
   ...
   Try { gm1=(const_cast<BaseMatrix&>(A)).Evaluate(); bx = gm1->IsZero(); }
...
bool CroutMatrix::IsEqual(const GeneralMatrix& A) const
{
   ...
   REPORT
   return RealEqual(A.store,store,storage)
      && intEqual(((const CroutMatrix&)A).indx, indx, nrows_value);
}
...
bool BandLUMatrix::IsEqual(const GeneralMatrix& A) const
{
   ...
   if ( A.Nrows() != nrows_value || A.Ncols() != ncols_value
      || ((const BandLUMatrix&)A).m1 != m1 || ((const BandLUMatrix&)A).m2 != m2 )
                                                 // different dimensions
   { REPORT  return false; }
   ...
   REPORT
   return RealEqual(A.Store(),store,storage)
      && RealEqual(((const BandLUMatrix&)A).store2,store2,storage2)
      && intEqual(((const BandLUMatrix&)A).indx, indx, nrows_value);
}


// ************************* quaternions ******************** //
ReturnMatrix MultiplyQuaternions(const ColumnVector& p, const ColumnVector& q) {
	float ans[] = {
...
ReturnMatrix ApplyQuaternion(const ColumnVector& q, const Matrix& m) {
	Matrix o(m.Nrows(),m.Ncols());
	const float t2 =   q(1)*q(2);
	const float t3 =   q(1)*q(3);
	const float t4 =   q(1)*q(4);
	const float t5 =  -q(2)*q(2);
	const float t6 =   q(2)*q(3);
	const float t7 =   q(2)*q(4);
	const float t8 =  -q(3)*q(3);
	const float t9 =   q(3)*q(4);
	const float t10 = -q(4)*q(4);
	for(int c = 1; c<=m.Ncols(); ++c) {
		o(1,c) = 2*( (t8 + t10)*m(1,c) + (t6 -  t4)*m(2,c) + (t3 + t7)*m(3,c) ) + m(1,c);
		o(2,c) = 2*( (t4 +  t6)*m(1,c) + (t5 + t10)*m(2,c) + (t9 - t2)*m(3,c) ) + m(2,c);
		o(3,c) = 2*( (t7 -  t3)*m(1,c) + (t2 +  t9)*m(2,c) + (t5 + t8)*m(3,c) ) + m(3,c);
	}
	if(m.Nrows()>3)
		o.SubMatrix(4,m.Nrows(),1,m.Ncols()) = m.SubMatrix(4,m.Nrows(),1,m.Ncols());
	o.Release(); return o;
}
