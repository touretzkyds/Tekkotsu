...
BandMatrix::BandMatrix(const BaseMatrix& M)
{
   ...
   GeneralMatrix* gmx=(const_cast<BaseMatrix&>(M)).Evaluate(MatrixType::BM);
...
UpperBandMatrix::UpperBandMatrix(const BaseMatrix& M)
{
   REPORT // CheckConversion(M);
   // MatrixConversionCheck mcc;
   GeneralMatrix* gmx=(const_cast<BaseMatrix&>(M)).Evaluate(MatrixType::UB);
   GetMatrix(gmx); CornerClear();
}
...
LowerBandMatrix::LowerBandMatrix(const BaseMatrix& M)
{
   REPORT // CheckConversion(M);
   // MatrixConversionCheck mcc;
   GeneralMatrix* gmx=(const_cast<BaseMatrix&>(M)).Evaluate(MatrixType::LB);
   GetMatrix(gmx); CornerClear();
}
...
BandLUMatrix::BandLUMatrix(const BaseMatrix& m)
{
   ...
   GeneralMatrix* gm = (const_cast<BaseMatrix&>(m)).Evaluate(MatrixType::BM);
...
//   while (i--) { sum *= *s; s += j; }
   ...
   const_cast<GeneralMatrix&>((const GeneralMatrix&)*this).tDelete(); return sum;
}
...
SymmetricBandMatrix::SymmetricBandMatrix(const BaseMatrix& M)
{
   REPORT  // CheckConversion(M);
   // MatrixConversionCheck mcc;
   GeneralMatrix* gmx=(const_cast<BaseMatrix&>(M)).Evaluate(MatrixType::SB);
   GetMatrix(gmx);
}
...
Real SymmetricBandMatrix::SumSquare() const
{
   ...
   const_cast<GeneralMatrix&>((const GeneralMatrix&)*this).tDelete(); return sum1 + 2.0 * sum2;
}
