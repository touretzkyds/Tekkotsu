...
public class TCPVisionListener extends TCPListener implements VisionListener {
    ...
    public void fireSensorUpdate() {
        updating=true;
        System.out.println("fireSensorUpdate() is updating " + listeners.size() + " listeners"); 
        for(int i=0; i<listeners.size() && updating; i++) {
            System.out.println("updateing listener " + i);
    ...
    public IndexColorModel getColorModel() { return usemodel; }
    ...
    byte[] _data=new byte[pktSize*2];
    byte[] _outd=new byte[pktSize*2];
    ...
    IndexColorModel cmodel; //original image, unfiltered
    IndexColorModel usemodel; //image after color converter has been used
    int pixel_size = 1;
    ...
    public void setReadingImage() { readingImage = true; }
    public boolean isReadingImage() { return readingImage; }
	...
	// System.out.println("int value creatorLen: " + creatorLen);
        if(!_isConnected) return ""; 
	String creator;
	if ( creatorLen > 0 && creatorLen < 1000 )
	    creator=new String(readBytes(in,creatorLen));
	else
	    creator = "Garbage in input";
        //System.out.println("string value creator: '" + creator + "'");
        if(!_isConnected) return "";
        if(readChar(in)!='\0') {
    ...
    public boolean readColorModel(InputStream in) throws java.io.IOException {
        ...
        if(cmodel==null || len!=cmodel.getMapSize() || ColorConverter.hasChanged) {
            makeNew=true;
        } else {
            int off=0;
            for(int i=0; i<len; i++) {
        ...
        if(makeNew) {
            //cmodel remains unconverted so it can be checked against the incoming image
            cmodel=new IndexColorModel(7,len,colormap,0,false);
            byte [] altermap = ColorConverter.convertRGBBytes(colormap);
            usemodel = new IndexColorModel(7, len, altermap, 0, false);
            if(ColorConverter.hasChanged){
                img=new BufferedImage(width,height,BufferedImage.TYPE_INT_RGB);
                ColorConverter.hasChanged = false;
            }
        }
        return true;
    }
    ...
    public boolean readSketchImage(InputStream in, int chanW, int chanH) throws java.io.IOException {
        ...
        if (sketchType == SKETCH_UCHAR_TYPE || sketchType == SKETCH_BOOL_TYPE)
            ...
            System.err.println("Format " + sketchType + " not recognized");
		...
		//System.out.println("Type ='" + type + "'");
                ...
                if (format!=oldformat || newWidth!=width || newHeight!=height) {
                    ...
                    synchronized (packetFormatChangeLock) {
                        ...
                        _data=new byte[pktSize*2];
                        _outd=new byte[pktSize*2];
                        _tmp=new byte[pktSize*2];
                ...
                for(int i=0; i<channels; i++) {
                    ...
                    if(!creator.equals("FbkImage")) {
                        ...
                        } else if (fmt.equals("DepthImage")) {
                            isJPEG=false;
                            isIndex=false;
                            int useChan=chan_id;
                            if(!readChannel(in,useChan,chanwidth,chanheight)) {
                                failed=true;
                                System.err.println("TCPVisionListener DepthImage channel read failed");
                                break;
                            }
    ...
    public int[] getIntensityPixels() {
        synchronized(packetFormatChangeLock) {
            ...
            int offset=0;
            if(!getConvertRGB()) {
                if (format == ENCODE_DEPTH) {
		    if ( jetred == null)
			makeJetMap();
                    for (int i=0; i<width*height; i++) {
                        int zvalue = (int)((data[offset++]&0xFF)) | (int)((data[offset++]&0xFF)<<8);
			// int zscaled = (z * 1023) / 10000;  // full range of Kinect zvalue is 0 to 10000
			int zscaled = (Math.min(zvalue,5000) * 1023) / 5000;  // focus on near objects: depth 0 to 5000
			int r = jetred[zscaled];
			int g = jetgreen[zscaled];
			int b = jetblue[zscaled];
    ...
    public static final int pixelYUV2RGB(int y, int u, int v) {
        u=u*2-256;
        v=v-128;
        int b=y+u;
        int r=y+v;
    ...
    public BufferedImage getImage() {
        ...
        if(isIndex) {
            ...
            if(usemodel==null)
                return img;
            // Load images for the SegCam viewer or SketchGUI.  The BufferedImage types don't really matter here; only their pixel size is important.
            if (pixel_size == 1) {
                if (img.getWidth()!=width || img.getHeight()!=height || img.getType()!=BufferedImage.TYPE_BYTE_INDEXED)
                    img=new BufferedImage(width,height,BufferedImage.TYPE_BYTE_INDEXED,usemodel);
                img.getRaster().setDataElements(0,0,width,height,data);
            } else if (pixel_size <= 4) {
                if(img.getWidth()!=width || img.getHeight()!=height || img.getType()!=BufferedImage.TYPE_4BYTE_ABGR)
                    img = new BufferedImage(width,height,BufferedImage.TYPE_4BYTE_ABGR);
                // Hack to get uint/usint data out of byte[] and into TYPE_4BYTE_ABGR
                int j=0;
                for (int i=0; i<img.getWidth()*img.getHeight(); i++) {
                    int x = i%img.getWidth(), y = i/img.getWidth(); 
                    img.getRaster().setSample(x,y,0,(int)data[j++]);
                    img.getRaster().setSample(x,y,1,(int)data[j++]);
                    if ( pixel_size > 2 ) {
                        img.getRaster().setSample(x,y,2,(int)data[j++]);
                        if ( pixel_size > 3 )
                            img.getRaster().setSample(x,y,3,(int)data[j++]);
                    }
                }
            } else {
                System.err.println("TCPVisionListener::getImage(): Unsupported pixel_size " + pixel_size);
            }
        } else {
            // Non-sketchGUI cases go here; we're sending RGB or YUV data (e.g., RawCam viewer)
            int[] pix;
    ...
    public void makeJetMap() {
        jetred = new int[1024];
        jetgreen = new int[1024];
        jetblue = new int[1024];
        ...
        for (int i=0; i<1024; i++) {
            if (i < 768)
                jetblue[i] = jetgreen[i+256];
            else
                jetblue[i] = 0;
                
            if (i >= 384)
                jetred[i] = jetgreen[i-256];
            else
                jetred[i] = 0;
        }

	//for (int i=0; i<1024; i+=16)
	// System.out.println(i + ":  r=" + jetred[i] + " g=" + jetgreen[i] + " b=" + jetblue[i]);

        // manually set color 0 to background gray
        jetred[0] = (int)Color.GRAY.getRed();
        jetgreen[0] = (int)Color.GRAY.getGreen();
        jetblue[0] = (int)Color.GRAY.getBlue();
    }
