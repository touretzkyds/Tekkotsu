...
int cmd_move(std::istream& is, std::ostream& os, const std::vector<std::pair<servoid,servoid> >& moveRanges, unsigned short movePos, unsigned short moveSpeed);

//! this version does the parsing, then calls the other version for execution
int cmd_move(std::istream& is, std::ostream& os, int argc, const char* argv[]) {
	...
	if ( !centered ) {
		if ( std::abs(moveAmount) < 1024 ) {
			movePos = moveAmount;
		} else {
			cerr << "ERROR: move value must be between 0 and 1023" << endl;
			return 2;
		}
	} else if ( moveAmount > 511 ) {
		cerr << "ERROR: signed moves must be between 0 and 512" << endl;
		return 2;
	} else {
		movePos = moveAmount + 512;
	}
	moveSpeed = ( i >= argc ) ? 128 : atoi(argv[i++]);
	
	return cmd_move(is, os, moveRanges, movePos, moveSpeed);
}
	...
	for(vector<pair<servoid,servoid> >::const_iterator it=moveRanges.begin(); it!=moveRanges.end(); ++it) {
		for(unsigned int sid=it->first; sid<=it->second; ++sid) {
			if(sid==DynamixelProtocol::BROADCAST_ID) {
				...
				DynamixelProtocol::write(os,DynamixelProtocol::SetPosSpeedCmd(sid,movePos,moveSpeed)).flush(); 
				void * res = ReadResponseThread(is,timeout,sid,"move").join();
...
static bool registered = registerCommand("move",cmd_move);
